...
export function loadPersistedConnectors(): import('./connector-types').GoContactConnector[] {
    try {
        ...
        return stored
            // Poll needs baseUrl (the instance); webchat-api can load without it
            // (it has its own apiBaseUrl) so a half-configured one stays editable
            // in the dashboard instead of silently vanishing.
            .filter(c => c.name && c.gocontact && (c.gocontact.baseUrl || c.gocontact.mode === 'webchat-api'))
            .map(c => {
